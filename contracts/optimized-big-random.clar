...
(define-data-var remaining uint u40)
...
(define-data-var TWO_FOLDS (list 2 uint) (list
    u0 u1
))

(define-public (init-vrf)
    (ok (var-set last-vrf (sha512 (unwrap-panic (get-block-info? vrf-seed (- block-height u1))))))
)
...
(define-public (test-element-at)
    (ok (do-element-at u0))
)

(define-private (do-element-at (byte-idx uint))
    (let ((vrf (sha512 (unwrap-panic (get-block-info? vrf-seed (- block-height u1))))))
        (concat
            (unwrap-panic (element-at vrf byte-idx))
            (unwrap-panic (element-at vrf (+ byte-idx u1)))
        )
    )
)

(define-public (add-bytes-test)
    (ok 
        (add-bytes
            (list u0 u1)
            (var-get last-vrf)
...
(define-private (add-bytes (ignores (list 2 uint)) (bytes (buff 64)))
    (fold add-bytes-iter ignores {acc: u0, factor: u256, bytes: bytes})
)

(define-private (add-bytes-iter (byte-pos uint) (output (tuple (acc uint) (factor uint) (bytes (buff 64)))))
    {
        acc: (+ 
            (get acc output)
            (* (unwrap-panic (index-of BUFF_TO_BYTE (unwrap-panic (element-at (get bytes output) byte-pos)))) (get factor output))
        ),
        factor: u1,
...
(define-public (mint)
    (let (
        ...
        (asserts! (> remaining-ids u0) (err u501))
        (ok (try! (cycle-random-id remaining-ids)))
    )
)
